...
class SecurityService {
    ...
    async startSecurityScan(containerId, imageName) {
        try {
    ...
    async getAllScans() {
        return Array.from(this.scans.values()).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    }
    async getContainerScans(containerId) {
    ...
    async evaluatePolicies(containerId, imageName, scanId) {
        try {
    ...
    async getAlerts(severity, status, limit = 100) {
    ...
    async getSecurityMetrics() {
        ...
        const passedComplianceChecks = completedScans.reduce((sum, s) => sum + s.compliance.filter(c => c.status === 'pass').length, 0);
    ...
    async performSecurityScan(scan) {
        try {
    ...
    async generateVulnerabilities(imageName) {
    ...
    calculateRiskScore(vulnerabilities, compliance) {
        ...
        const vulnerabilityScore = vulnerabilities.reduce((sum, vuln) => sum + severityWeights[vuln.severity], 0);
    ...
    async evaluateRule(rule, containerId, imageName, scan) {
