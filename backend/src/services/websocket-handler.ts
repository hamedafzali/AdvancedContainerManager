...
export class WebSocketHandler {
  ...
  public handleConnectionPublic(socket: Socket): void {
    this.handleConnection(socket);
  }
  ...
  private setupClientHandlers(socket: Socket): void {
    ...
    socket.on("subscribe_container", (data) => {
      ...
      this.logger.info(
        `Client ${socket.id} subscribed to container ${containerId}`,
      );
    });
    ...
    socket.on("unsubscribe_container", (data) => {
      ...
      this.logger.info(
        `Client ${socket.id} unsubscribed from container ${containerId}`,
      );
    });
    ...
    socket.on("get_container_metrics", async (data) => {
      try {
        const { containerId } = data;
        const metrics =
          await this.metricsCollector.collectContainerMetrics(containerId);
        socket.emit("container_metrics", { containerId, metrics });
      } catch (error) {
        const { containerId } = data;
        this.logger.error(
          `Error getting container metrics for ${containerId}:`,
          error,
        );
        socket.emit("error", {
    ...
    socket.on("get_system_metrics_history", async (data) => {
      try {
        ...
        const history =
          await this.metricsCollector.getSystemMetricsHistory(limit);
    ...
    socket.on("get_container_metrics_history", async (data) => {
      try {
        ...
        const history = await this.metricsCollector.getContainerMetricsHistory(
          containerId,
          limit,
        );
        ...
        const { containerId } = data;
        this.logger.error(
          `Error getting container metrics history for ${containerId}:`,
          error,
        );
        socket.emit("error", {
